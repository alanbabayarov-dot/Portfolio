...
- **`.slot img`/`.slot video` are `position:absolute;inset:0` by design.** iOS Safari resolves `height:100%` against `aspect-ratio`-sized parents unreliably when the media lazy-loads — in-flow media would render at natural size and "blow up" out of the slot mid-scroll (the recurring Toolor-block bug). Absolute fill means a media load can never touch layout. Consequence: **every `.slot` must carry its own size** (aspect-ratio, height, or grid/flex stretch). The single exception is `.wide-solo .slot img` (`position:static;height:auto`) — those slots are sized by the image, which is safe only because those imgs have `width`/`height` attributes.
